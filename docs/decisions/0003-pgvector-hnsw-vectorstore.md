...
> **한 줄 요약(BLUF):** 벡터 저장·검색을 **PostgreSQL pgvector + HNSW**(`vector_cosine_ops`)로 한다. 전용 벡터 DB(Milvus 등) 대신 단일 Postgres에 dense(pgvector)와 sparse(tsvector)를 함께 두어 인프라를 단순화한다.

- **Status:** Accepted
- **Date:** 2026-06-27 (소급)
- **근거 코드:** `src/db/vector_store.py` · **관련:** [archive](../archive/README.md)(Milvus·AGE 폐기 경위)
