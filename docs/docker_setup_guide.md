...
- **다목적 데이터베이스 결합**: 벡터 검색(`pgvector`)과 그래프 DB(`Apache AGE`)를 지원하는 커스텀 PostgreSQL 컨테이너를 함께 구동합니다.
...
과거에는 수동으로 각 환경에 접속하여 확장 기능(`vector`, `age`)의 설치 여부나 앱의 종속성 상태를 점검했으나, 
...
# psql 내부에서 가능한 작업 예시:
# 1. AGE 확장이 필요한 SQL 실행이나 그래프 노드 구조체 점검
