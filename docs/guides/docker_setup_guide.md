...
| `TEI_MAX_BATCH_TOKENS` | `8192` | 저사양 호스트(RAM 16GB 이하 VM)에서 웜업 메모리 부족(OOM, exit 137)이 발생할 때 `4096`으로 낮춘다. |
...
TEI 임베딩 컨테이너는 기동 시 배치 상한 크기의 더미 텐서로 웜업을 진행하므로, 저사양 CPU 호스트(RAM 16GB 이하)에서는 `TEI_MAX_BATCH_TOKENS=4096` 설정을 권장합니다. 입력을 조용히 절단하는 `--auto-truncate` 옵션은 적재 토큰 상한 규약(IX-201)과 어긋나므로 사용하지 않으며, `--max-input-length 4096`을 명시하여 안정적으로 기동합니다.
