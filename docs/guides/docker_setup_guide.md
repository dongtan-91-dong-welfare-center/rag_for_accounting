...
TEI 임베딩 컨테이너는 기동 시 배치 상한 크기의 더미 텐서로 웜업을 진행하므로, 저사양 CPU 호스트(RAM 16GB 이하)에서도 OOM 없이 기동할 수 있도록 기본값을 `TEI_MAX_BATCH_TOKENS=4096`으로 설정하였습니다. 입력을 조용히 절단하는 `--auto-truncate` 옵션은 적재 토큰 상한 규약(IX-201)과 어긋나므로 사용하지 않으며, `--max-input-length 4096`을 명시하여 안정적으로 기동합니다.
