...
[판정 기준] 기준의 정본은 `scripts/rerank_replay.py`의 judge_adoption()이며, 이 하니스는 그 함수를 그대로 호출한다(k=RRF_K 고정 판정).
수치를 여기 옮겨 적으면 기준 개정 때 낡은 서술이 남는다.
  이 하니스가 남긴 #211 2차 리포트는 구 기준(회귀 0건·순증 ≥2건 고정)으로 판정된 것이다.
  정성으로는 1차가 해친 004·007·010·014가 회복되는지, 1차가 못 구제한 미검출 5건
  (특히 011 — 질의 "순액으로만 표시"에 없는 "상계"를 키워드가 넣어야 잡힌다)을 본다.
