...
[채택/롤백 기준] 기준의 정본은 `scripts/rerank_replay.py`의 judge_adoption()이며, 이 하니스는 그 함수를 그대로 호출한다(k=RRF_K 고정 판정).
수치를 여기 옮겨 적지 않는 이유는 기준이 바뀔 때 하니스마다 서술이 낡아 서로 다른 기준을 말하게 되기 때문이다.
  다만 이 하니스가 처음 남긴 리포트(#211 1차)는 구 기준(회귀 0건·순증 ≥2건 고정)으로 판정된 것이므로, 지금 다시 돌리면 개정된 기준으로 결론이 달라질 수 있다.
