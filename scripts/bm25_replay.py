...
[배경] 이 시스템의 sparse 검색이 쓰는 PostgreSQL ts_rank_cd에는 IDF(자주 나오는 단어일수록 가중치를 자동으로 낮추는 값)가 없다 — ts_rank_cd는 BM25가 아니기 때문이다.
...
[판정 기준] 기준의 정본은 `scripts/rerank_replay.py`의 judge_adoption()이며, 이 하니스는 그 함수를 그대로 호출한다(RRF k=60 고정).
수치를 여기 옮겨 적으면 기준 개정 때 낡은 서술이 남으므로 적지 않는다.
이 하니스가 남긴 #226 리포트는 구 기준(회귀 0건·순증 ≥2건 고정·모집단 11건)으로 판정된 것이므로, 벤치마크가 114건으로 늘고 기준이 개정된(2026-07-25) 지금 다시 돌리면 결론이 달라질 수 있다.
